...
const PunchCamera = (props, ref) => {
     const { usersURL } = useContext(URLContext);
     const { editedEmployee, setEditedEmployee, allEmployees, setAllEmployees, isLoading, isReload } = useContext(DataContext);
     ...
     const loadLabelImages = () => {
          console.log(allEmployees);
          var employees = allEmployees.filter((employee) => employee.image.length > 0);
          return Promise.all(
               ...
               if (detections.length > 1) {
                    ...
                    if (detections.length === 1 && detections[0].detection._score > 0.65) {
                         ...
                         if (results[0]._label !== "unknown") {
                              ...
                              props.setThisEmployee({ ...parseResult, time: props.timer.toLocaleTimeString() });
                              // props.setThisEmployee({ ...JSON.parse(results[0]._label), time: props.timer.toLocaleTimeString("en-US") });

                              // const data = { date: props.timer.toLocaleDateString("en-CA") };
                              // data[props.punch.status] = props.timer.toLocaleTimeString();
